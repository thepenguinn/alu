...
\newcommand\constcorner[2] {
    ...
    (curtmp#1finaltmp) coordinate (LCVTMP)
    ($(LCVTMP)!\cornerbase!0:(finaltmp)$) coordinate (SCBTMP)
    ($(LCVTMP)!\cornerbase!0:(curtmp)$) coordinate (FCBTMP)

    (curtmp) -- (FCBTMP) -- (SCBTMP)
}
...
\newcommand\markvarcornerpos[2] {
    ...
    (curtmp#1SCBTMP) coordinate (LCVTMP)
    ($(SCBTMP)!0.1cm!-45:(LCVTMP)$) coordinate (cornedgtmp)
    (intersection of curtmp--LCVTMP and finaltmp--cornedgtmp)
    coordinate (FCBTMP)
}
