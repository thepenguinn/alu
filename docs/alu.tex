...
\newcommand\constcorner[2] {
    ...
    (curtmp#1FinalCornerTmp) coordinate (LCTmp)
    ($(LCTmp)!\cornerbase!0:(FinalCornerTmp)$) coordinate (sectmp)
    ($(LCTmp)!\cornerbase!0:(curtmp)$) coordinate (fsttmp)
...
\begin{circuitikz}[american]
    ...
    (ainit-Bout)
    \constcornerhoz {++(0.75,1)}
    \slightup

    \constcornervrt {(ainitBoutheight)}
    \slightleft
    ;
