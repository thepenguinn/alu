...
\ctikzsubcircuitdef{subctkfullAdder} {
    ...
    (#1.top left) ++(-0.25,0)
    coordinate (#1-lpad)
    (#1.bottom right) ++(0.25,0)
    coordinate (#1-rpad)
    (#1.top left) ++(0,0.25)
    coordinate (#1-tpad)
    (#1.bottom right) ++(0,-0.25)
    coordinate (#1-bpad)
}
...
\begin{circuitikz}[american]
    ...
    coordinate (fladBinsecbend)
    \constcornerdown {(flad-B)}
    coordinate (fladjustBin)
    -- (flad-B)

    (msff-Q) -- ++(0.75,0)
    coordinate (msqout)

    -- (msqout|-ainitBoutheight)

    \constcornerup {++(-1,0.75)}
    \aroundcornerleftdown {0.75} {(fladBinsecbend)}

    \aroundcornertilldownright {0.75} {(fladjustBin)} {(flad-Cin)}
    -- (flad-Cin)
